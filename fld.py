...
class FLD(object):
    ...
    def evaluate(self):
        ...
        for batch_id, samples in enumerate(data_loader):
            ...
            if (
                hasattr(config.test_param, "refine_300W")
                and config.test_param.refine_300W
            ):
                landmarks_pred = refine_300W_landmarks(
                    samples["image_cv2"].numpy(), landmarks_pred
                )
            ...
            if config.visualize:
                ...
                save_result_imgs(
                    images_cv2, self.vis_dir, names, landmarks_pred, landmarks_targ
                )
    ...
    def eval_ckpts(self):
        ...
        csv_helper = CsvHelper(
            head=["Name", "ION", "IPN"], save_path=os.path.join(expname, "ckpts.csv")
        )
        ...
        csv_helper.save()
        logger.info(
            "Min ION: {ION.min:.4f} " "Min IPN: {IPN.min:.4f} ".format(ION=ION, IPN=IPN)
        )
        ...
        if hasattr(config, "load_path") and config.load_path:
            if os.path.isfile(config.load_path):
                if hasattr(config, "resume") and config.resume:
                    self.start_epoch = load_model(
                        config.load_path, self.model, self.optimizer
                    )
                else:
    ...
    def _build_criterion(self):
        ...
        self.laplace_loss = LaplacianLoss(
            WiderWingLoss(**config.kwargs).cuda(), num_points=num_points
        ).cuda()
