...
class Refinement(nn.Module):
    def __init__(self, ku, kh, kd, upscale):
        ...
        self.conv_h = nn.Conv2d(
            in_channels=kh, out_channels=kh // 2, kernel_size=3, padding=1
        )
        self.bn_h = nn.BatchNorm2d(kh // 2)

        self.conv_u = nn.Conv2d(
            in_channels=ku, out_channels=ku // 2, kernel_size=3, padding=1
        )
        self.bn_u = nn.BatchNorm2d(ku // 2)

        self.conv_d = nn.Conv2d(
            in_channels=kh // 2 + ku // 2, out_channels=kd, kernel_size=3, padding=1
        )
        ...
        if upscale != 1:
            self.conv_sub = nn.Conv2d(
                in_channels=kd,
                out_channels=kd * upscale * upscale,
                kernel_size=3,
                padding=1,
            )
