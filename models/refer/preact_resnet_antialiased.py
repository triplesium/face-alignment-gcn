...
class PreActBlockAntiAliasedIBN_a(PreActBlockAntiAliased):
    ...
    def __init__(self, in_planes, planes, stride=1, filter_size=3):
        super(PreActBlockAntiAliasedIBN_a, self).__init__(
...
class PreActResNetAntiAliased(PreActResNetIBN):
    ...
    def __init__(
        ...
        super(PreActResNetAntiAliased, self).__init__(
            blocks=blocks,
        ...
        if not pool_only:
            self.conv1 = nn.Conv2d(
                3, num_feats[0], kernel_size=7, stride=1, padding=3, bias=False
            )
...
def PreActResNetAntiAliased18(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
    cprint.green("Creating PreActResNetAntiAliased18 ......")
    blocks = [
        PreActBlockAntiAliasedIBN_a,
        PreActBlockAntiAliasedIBN_a,
        PreActBlockAntiAliasedIBN_a,
        PreActBlockAntiAliased,
    ]

    return PreActResNetAntiAliased(
        blocks=blocks,
        num_blocks=[2, 2, 2, 2],
...
def PreActResNetAntiAliased18V2(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
