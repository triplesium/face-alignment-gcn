...
class PreActResNetIBN_att(PreActResNet):
    def __init__(
        self, blocks, num_blocks, num_feats, is_color=True, receptive_keep=False
    ):
        ...
        self.layer4 = self._make_layer(
            blocks[3], num_feats[3], num_blocks[3], stride=stride
        )
...
def PreActResNet34IBN_att(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
    cprint.green("Creating PreActResNet34IBN_att ......")
    blocks = [PreActBlockIBN_a, PreActBlockIBN_a, PreActBlockIBN_a, PreActBlock]
    model = PreActResNetIBN_att(
        blocks=blocks,
        num_blocks=[3, 4, 6, 3],
        num_feats=[64, 128, 256, 512],
        is_color=is_color,
...
def PreActResNet50IBN_att(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
    cprint.green("Creating PreActResNet50IBN_att ......")
    blocks = [
        PreActBottleneckIBN_a,
        PreActBottleneckIBN_a,
        PreActBottleneckIBN_a,
        PreActBottleneck,
    ]
...
class PreActResNetIBN_HeatmapAtt(PreActResNet):
    def __init__(
        ...
        self.att = nn.Sequential(
            nn.Conv2d(
                num_feats[0] * expansion, 256, kernel_size=3, padding=1, stride=1
            ),
            nn.ReLU(True),
...
def PreActResNet18IBN_HeatmapAtt(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
    cprint.green("Creating PreActResNet18IBN_HeatmapAtt ......")
    blocks = [PreActBlockIBN_a, PreActBlockIBN_a, PreActBlockIBN_a, PreActBlock]
    return PreActResNetIBN_HeatmapAtt(
        blocks=blocks,
        num_blocks=[2, 2, 2, 2],
        num_feats=[64, 128, 256, 512],
        is_color=is_color,
...
def PreActResNet34IBN_HeatmapAtt(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
    cprint.green("Creating PreActResNet34IBN_HeatmapAtt ......")
    blocks = [PreActBlockIBN_a, PreActBlockIBN_a, PreActBlockIBN_a, PreActBlock]
    model = PreActResNetIBN_HeatmapAtt(
        blocks=blocks,
        num_blocks=[3, 4, 6, 3],
        num_feats=[64, 128, 256, 512],
        is_color=is_color,
...
def PreActResNet50IBN_HeatmapAtt(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
...
def PreActResNet18IBN_CHatt(
    is_color, pretrained_path=None, receptive_keep=False, **kwargs
):
    cprint.green("Creating PreActResNet18IBN_CHatt ......")
    blocks = [PreActBlockIBN_a, PreActBlockIBN_a, PreActBlockIBN_a, PreActBlock]
    return PreActResNetIBN_CHatt(
        blocks=blocks,
        num_blocks=[2, 2, 2, 2],
        num_feats=[64, 128, 256, 512],
        is_color=is_color,
