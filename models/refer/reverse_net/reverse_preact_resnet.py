...
class PreActResNetBottomUp(PreActResNet):
    def __init__(self, block, num_blocks, num_feats, is_color=True):
        super(PreActResNetBottomUp, self).__init__(
            block, num_blocks, num_feats, is_color=True, receptive_keep=False
        )
...
class PreActResNetTopDown(nn.Module):
    def __init__(self, block, num_blocks, num_feats, num_boundary=1):
        ...
        self.conv_sub = nn.Conv2d(
            ...
            out_channels=self.in_planes * upscale,
            kernel_size=3,
            padding=1,
        )
        ...
        self.refine3 = Refinement(
            ku=num_feats[2] * block.expansion,
            kh=num_feats[2] * block.expansion,
            kd=num_feats[1] * block.expansion,
            upscale=upscale,
        )

        self.layer2 = self._make_layer(block, num_feats[1], num_blocks[1], stride=1)
        self.refine2 = Refinement(
            ku=num_feats[1] * block.expansion,
            kh=num_feats[1] * block.expansion,
            kd=num_feats[0] * block.expansion,
            upscale=upscale,
        )

        self.layer1 = self._make_layer(block, num_feats[0], num_blocks[0], stride=1)
        self.refine1 = Refinement(
            ku=num_feats[0] * block.expansion,
            kh=num_feats[0] * block.expansion,
            kd=num_feats[0] * block.expansion // 2,
            upscale=1,
        )

        self.conv = nn.Conv2d(
            in_channels=num_feats[0] * block.expansion // 2,
            out_channels=num_boundary,
            kernel_size=3,
            padding=1,
        )
        self._init_weights()
...
class PreActResUNet(nn.Module):
    def __init__(self, block, num_blocks, num_feats, is_color=True, num_boundary=1):
        ...
        self.bottom_up = PreActResNetBottomUp(
            block, num_blocks, num_feats, is_color=is_color
        )
        num_top_down_blocks = [1] * len(num_blocks)
        self.top_down = PreActResNetTopDown(
            block, num_top_down_blocks, num_feats, num_boundary
        )
