...
class MultiBranchPredictor(nn.Module):
    def __init__(self, in_channels, feat_size, num_points, **kwargs):
        ...
        self.conv_contour = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )  # contour
        self.conv_left_eyebrow = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )  # left top eyebrow + left bottom eyebrow (98pt)
        self.conv_right_eyebrow = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )  # right top eyebrow + right bottom eyebrow (98pt)
        self.conv_nose = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )  # nose bridge + nose tip
        self.conv_left_eye = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )  # left top eye + left bottom eye
        self.conv_right_eye = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )  # right top eye + right bottom eye
        self.conv_lip = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )  # up up lip + up bottom lip + bottom up lip + bottom bottom lip
        self.conv_left_eyebrow_b = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )
        self.conv_right_eyebrow_b = nn.Conv2d(
            in_channels, in_channels, kernel_size=feat_size
        )
