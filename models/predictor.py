...
from models.gcn_model.map_to_node import (
    MapToNode,
    MapToNode2b,
    MapToNodeAttention,
...
class MultiDenseGCNPredictor(nn.Module):
    def __init__(self, in_channels, num_points, feat_size, **kwargs):
        ...
        self.adjacency_learner_in = AdjacencyLearner(
            in_channels=512, feat_size=8, num_points=num_points
        )
        self.adjacency_learner1 = AdjacencyLearner(
            in_channels=512, feat_size=8, num_points=num_points
        )
        self.adjacency_learner2 = AdjacencyLearner(
            in_channels=512, feat_size=8, num_points=num_points
        )
        self.adjacency_learner3 = AdjacencyLearner(
            in_channels=512, feat_size=8, num_points=num_points
        )
        self.adjacency_learner4 = AdjacencyLearner(
            in_channels=512, feat_size=8, num_points=num_points
        )
        self.adjacency_learner_out = AdjacencyLearner(
            in_channels=512, feat_size=8, num_points=num_points
        )

        hid_dim = kwargs["hid_dim"] if "hid_dim" in kwargs else 128
        num_layers = kwargs["num_layers"] if "num_layers" in kwargs else 4
        self.g_net = MultiDenseGCNet(
            in_channels=64 * 4, out_channels=2, hid_channels=hid_dim
        )
...
class MultiHeadSemGraphPredictorV2b(nn.Module):
    def __init__(self, in_channels, num_points, feat_size, **kwargs):
        ...
        self.sem_gcn = MultiHeadSemGCNet(
            adj_matrix,
            in_channels=64 * 4,
            out_channels=2,
            hid_channels=hid_dim,
...
class StructureCoherencePredictor(nn.Module):
    def __init__(self, in_channels, num_points, feat_size, **kwargs):
        ...
        top_k = kwargs["top_k"] if "top_k" in kwargs else False
        expansion_factor = (
            kwargs["expansion_factor"] if "expansion_factor" in kwargs else 4
        )
        self.map_to_node = MapToNodeAttention(
        ...
        self.sem_gcn = SemGCN(
            ...
            hid_dim=hid_dim,
            num_layers=num_layers,
            coords_dim=(64 * expansion_factor, 2),
            p_dropout=p_dropout,
        )
...
class MultiFeaturePredictor(nn.Module):
    ...
    def __init__(self, in_channels, feat_size, num_points, **kwargs):
        ...
        self.conv_1 = nn.Conv2d(
            in_channels[-1], 32, kernel_size=3, padding=1, stride=2
        )  # before: feat_size, after: feat_size//2
...
def SemChGraphPredictor(in_channels, num_points, feat_size, **kwargs):
    ...
    return _SemChGraphPredictor(
        map_to_node=MapToNode,
        in_channels=in_channels,
        feat_size=feat_size,
        num_points=num_points,
    )
...
class DynamicGCNetPredictor(nn.Module):
    ...
    def __init__(self, in_channels, num_points, feat_size, **kwargs):
        ...
        hid_dim = kwargs["hid_dim"] if "hid_dim" in kwargs else 128
        num_layers = kwargs["num_layers"] if "num_layers" in kwargs else 4
        self.g_net = DynamicGCNet(
            adj_matrix,
            in_channels=feat_size**2 * 4,
            out_channels=2,
            num_points=num_points,
            hid_channels=hid_dim,
            top_k=top_k,
...
class DynamicGCNetPredictorV2(nn.Module):
    ...
    def __init__(self, in_channels, num_points, feat_size, **kwargs):
        ...
        self.map_to_node = MapToNodeAttentionV2(
...
class SelfPredictor(nn.Module):
    ...
    def __init__(self, in_channels, num_points, feat_size, **kwargs):
        ...
        hid_dim = kwargs["hid_dim"] if "hid_dim" in kwargs else 128
        num_layers = kwargs["num_layers"] if "num_layers" in kwargs else 4
        self.g_net = SemSelfGCN(
            hid_dim=hid_dim, coords_dim=(feat_size**2 * 4, 2), num_layers=num_layers
        )
...
class DepthwiseSeparableDynamicGCNetPredictor(nn.Module):
    def __init__(self, in_channels, num_points, feat_size, **kwargs):
        super(DepthwiseSeparableDynamicGCNetPredictor, self).__init__()
        logger.debug("build DepthwiseSeparableDynamicGCNetPredictor ......")
        self.map_to_node = DepthwiseSeparableMapToNode(
            in_channels=in_channels, num_points=num_points
        )
        top_k = kwargs["top_k"] if "top_k" in kwargs else False
