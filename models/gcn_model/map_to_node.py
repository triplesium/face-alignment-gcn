...
class MapToNode2b(nn.Module):
    ...
    def __init__(self, in_channels, num_points):
        ...
        self.conv34 = nn.Sequential(
            nn.Conv2d(
                outfeat4nb + outfeat3nb, outfeat3nb, kernel_size=1, padding=0, stride=1
            ),
            nn.BatchNorm2d(outfeat3nb),
            nn.ReLU(inplace=True),
        )
        self.conv234 = nn.Sequential(
            nn.Conv2d(
                outfeat2nb + outfeat3nb, outfeat2nb, kernel_size=1, padding=0, stride=1
            ),
            nn.BatchNorm2d(outfeat2nb),
            nn.ReLU(inplace=True),
        )
        ...
        self.conv_to_node = nn.Sequential(
            nn.Conv2d(
                outfeat2nb + outfeat3nb + outfeat4nb,
                out_featnb,
                kernel_size=1,
                padding=0,
                stride=1,
