...
def eval_NME(pred, targ, num_points=68, mode="IO"):
    ...
    mse = np.mean(
        np.sqrt(np.sum((pred_vec2 - targ_vec2) ** 2, axis=2)), axis=1
    )  # batchsize
...
def eval_NME_torch(pred: torch.Tensor, targ: torch.Tensor, num_points=68, mode="IO"):
    """Evaluation metric for face alignment, torch version
    Args:
        pred (torch.Tensor - shape of batchsize x num_points*2 x 1): predicted face landmarks
        targ (torch.Tensor - shape of batchsize x num_points*2 x 1): target face landmarks
        num_points (int): # face landmarks
        mode (string): 'IP' inter pupil
                       'IO' inter ocular
    Return:
    ...
    pred_vec2 = pred.view(-1, num_points, 2)  # batchsize x 68 x 2
    targ_vec2 = targ.view(-1, num_points, 2)

    le_loc = torch.mean(targ_vec2[:, le_idx, :], axis=1)  # batchsize x 2
    re_loc = torch.mean(targ_vec2[:, re_idx, :], axis=1)

    norm_dist = torch.sqrt(torch.sum((le_loc - re_loc) ** 2, axis=1))  # batchsize
    mse = torch.mean(
        torch.sqrt(torch.sum((pred_vec2 - targ_vec2) ** 2, axis=2)), axis=1
    )  # batchsize

    nme = mse / norm_dist

    return torch.mean(nme), nme
