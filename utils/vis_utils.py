...
def get_show_img(config, input_img, show_idx):
    ...
    show_img = (
        ((sample_img * stds + means) * img_norm_factor)
        .permute(2, 0, 1)
...
def save_result_img(image, landmark_pred, landmark_targ, save_path):
    ...
    for i in range(num_kpts):
        targ_pt = (
            round(float(landmark_targ[2 * i])),
            round(float(landmark_targ[2 * i + 1])),
        )
        pred_pt = (
            round(float(landmark_pred[2 * i])),
            round(float(landmark_pred[2 * i + 1])),
        )
