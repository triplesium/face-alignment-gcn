...
def save_checkpoint(
    state, checkpoint="checkpoint", filename="checkpoint.pth.tar", is_best=False
):
...
def load_model(ckpt_obj, model, optimizer=None):
    if isinstance(ckpt_obj, dict):
        ...
        checkpoint = torch.load(
            ckpt_obj, map_location=lambda storage, loc: storage.cuda(device)
        )
...
def get_checkpoints(root):
    files = [
        ...
        if file.endswith(".pth.tar") and file.startswith("checkpoint_")
    ]
