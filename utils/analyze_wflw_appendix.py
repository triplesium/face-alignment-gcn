...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="/mnt/lustre/share/zhubeier/WFLW/test/test_98pt_attr.txt",
    )
...
def main():
    ...
    with open(save_path, "w") as f:
        ...
        for i in range(len(Subset)):
            subset_string = "{} ION: {:.4f} NUM: {}".format(
                Name[i], Subset[i].avg, Subset[i].count
            )
            print(subset_string)
        ...
        for i in range(len(Subset)):
            subset_string = "{} FR: {:.4f} NUM: {}".format(
                Name[i], Subset_FR[i].avg, Subset_FR[i].count
            )
            print(subset_string)
            f.write(subset_string + "\n")
