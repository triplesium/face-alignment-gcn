...
def get_transform(center, w, h, rot, res_w, res_h):
    ...
    t[1, 1] = scale_h
    t[2, 2] = 1
    t[0, 2] = -float(center[0]) * scale_w + 0.5 * res_w
    t[1, 2] = -float(center[1]) * scale_h + 0.5 * res_h
    if not rot == 0:
...
def kpt_transform(pts, center, box_w, box_h, rot, res_w, res_h, invert):
    ...
    if invert:
        t = np.linalg.inv(t)
    new_pt = np.array([pts[0], pts[1], 1.0]).T
...
def refine_300W_landmarks(images, landmarks_pred):
    ...
    for i, (image, landmark_pred) in enumerate(zip(images, landmarks_pred)):
        ...
        for j in range(len(landmark_pred) // 2):
            landmarks_refine[i, 2 * j] = (
                landmark_pred[2 * j] if landmark_pred[2 * j] > l else l
            )
            landmarks_refine[i, 2 * j] = (
                landmark_pred[2 * j] if landmark_pred[2 * j] < r else r
            )
            landmarks_refine[i, 2 * j + 1] = (
                landmark_pred[2 * j + 1] if landmark_pred[2 * j + 1] > t else t
            )
            landmarks_refine[i, 2 * j + 1] = (
                landmark_pred[2 * j + 1] if landmark_pred[2 * j + 1] < b else b
            )
