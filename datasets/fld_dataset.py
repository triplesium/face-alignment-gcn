...
import cv2
from datasets.align_transforms import (
    RandomAffine,
    CenterCrop,
    Grayscale,
    RandomOcclude,
...
class TransformBuilder(object):
    ...
    def train_transforms(self):
        ...
        train_transforms.append(
            RandomAffine(
                degrees=config.train_param.rotation,
                translate=(
                    config.train_param.translate_rate,
                    config.train_param.translate_rate,
                ),
                scale=(config.train_param.scale_min, config.train_param.scale_max),
                mirror=config.train_param.mirror,
                corr_list=cor_list,
            )
        )
        train_transforms.append(CenterCrop(config.structure.crop_size))
        if (
            hasattr(config.train_param, "is_occluded")
            and config.train_param.is_occluded
        ):
        ...
        if (
            hasattr(config.train_param, "use_boundary")
            and config.train_param.use_boundary
        ):
...
class FLDDS(data.Dataset):
    ...
    def __init__(
        ...
        assert self.num_points in [29, 68, 98, 106], (
            "#landmarks number expected to be {29, 68, 98, 106}, got %d"
            % self.num_points
        )
        ...
        for root_folder, source in zip(self.root_list, self.source_list):
            ...
            for line in lines:
                ...
                elements = line.strip("\n").strip(" ").split(" ")
                assert (
                    len(elements) in valid_anno_num
