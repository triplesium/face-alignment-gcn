...
def _fit_points_in_boundary(points):
    ...
    assert num_pts in [68, 98, 106], (
        "#landmarks should be 68, 98 or 106, but got %d" % num_pts
    )

    if num_pts == 68:
        boundary_index = boundary_index_68pts
    if num_pts == 98:
        boundary_index = boundary_index_98pts
    if num_pts == 106:
        boundary_index = boundary_index_106pts
...
def _fit_boundary_in_part(mode, num_pts):
    ...
    if mode == "individual":
        ...
        raise Exception("Invalid mode, should be individual, global or group", mode)
...
def _filter_valid_point(points, epsilon):
    ...
    if len(valid_pts) <= 2:
        print(
            "[WARNING]#valid points should greater than 2, but got %d" % len(valid_pts)
        )
...
def landmarks_to_boundary_heatmap(
    points, arrange_mode, heatmap_size=(384, 384), label_size=(256, 256), sigma=1
):
    ...
    points = list(
        map(
            lambda pt: pt
            * Vec2(heatmap_size[1] / label_size[1], heatmap_size[0] / label_size[0]),
            points,
        )
    )
    num_pts = len(points)
    ...
    for i in range(num_boundary):
        ...
        for pt in points_interpolated:
            ...
            if (
                int_x > 0
                and int_x < heatmap_size[1]
                and int_y > 0
                and int_y < heatmap_size[0]
            ):
                B[part_list[i], int_y, int_x] = 0

    for i in range(num_channels):
        D = cv2.distanceTransform(
            B[i, :, :], cv2.cv2.DIST_L2, cv2.cv2.DIST_MASK_PRECISE
        )
...
def landmarks_to_landmark_heatmap(points, heatmap_size=64, label_size=256, sigma=1.0):
    points = _floats2Vecs(points)
    # reshape points label
    points = list(
        map(
            lambda pt: pt * Vec2(heatmap_size / label_size, heatmap_size / label_size),
            points,
        )
    )
    num_pts = len(points)
